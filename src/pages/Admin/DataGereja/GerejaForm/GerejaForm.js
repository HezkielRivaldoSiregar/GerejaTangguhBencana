...
function GerejaForm () {
  ...
  const { data: church, isSuccess, error, isLoading } = useQuery(() => {
    if (id) {
      return axios.get(`http://localhost:8080/api/churches/${id}`)
        .then(res => res.data)
    }
  }, { isEnabled: isEdit })
            ...
            : (
              <Form
                ...
                onSubmit={(e) => {
                  ...
                  if(isEdit) {
                    axios.put(`http://localhost:8080/api/churches/${id}?` + query).then(() => {
                    ...
                    axios.post(`http://localhost:8080/api/churches?` + query).then(() => {
